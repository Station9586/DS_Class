...
class TwoThreeTree(object):
    ...
    def inorder (self, p, c = ""): 
        if (p is None): return;
        # print(p.values, c, end ="\n");
        out = [];
        for i in p.values: 
            if (i is None): break;
            out.append(i);
        print(out, c, end = "\n");
        if (p.refcnt >= 1 and p.links[0] != None): self.inorder(p.links[0], c + "L");
        if (p.refcnt >= 2 and p.links[1] != None): self.inorder(p.links[1], c + "M");
        if (p.refcnt >= 3 and p.links[2] != None): self.inorder(p.links[2], c + "R");

def Print (p, c = ""):
    print(c + str(p));
    if (p is not None):
        for i in range(p.refcnt): Print(p.getLink(i), c + "  ");

# tree = TwoThreeTree();

# num = [45, 30, 10, 20, 40, 70, 50, 80, 85]

# for i in num: tree.insertValue(i);
